...
const Dinosaur = (props) => {
    ...
    // console.log(useBox())
    // const group = useRef()
    const { nodes } = useLoader(GLTFLoader, "/assets/3D/Dinosaur/scene.gltf")
    const model = nodes.trex_trex_regular_0
    const texture = useLoader(THREE.TextureLoader, '/assets/3D/Dinosaur/textures/trex_regular_baseColor.jpeg')
    texture.flipY=false;
